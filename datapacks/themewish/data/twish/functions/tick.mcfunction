...
scoreboard players enable @a deletethemewish
...
execute as @a[scores={deletethemewish=1..9}] at @e[type=minecraft:armor_stand,tag=twishstand] run function twish:deletewishes
execute as @a[scores={deletethemewish=10..}] run tellraw @s ["",{"text":"Das ist keine gültige ID (1-9 erlaubt)","color":"red"}]
